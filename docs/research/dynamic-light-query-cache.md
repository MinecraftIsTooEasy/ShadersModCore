...
3. `ChunkCacheMixin.modifyCout` 在该方法返回后检查动态光照，并调用 `DynamicLights.getCombinedLight(BlockPos,int)`。这里每次都会创建一个 `BlockPos` record，并遍历当前动态光照快照。
...
在 `DynamicLights` 增加固定大小、线程本地的坐标查询缓存，并只从 `ChunkCacheMixin` 的动态光照分支调用它。键包含 `(x,y,z,原始合并亮度,更新纪元)`；未命中仍执行原有计算，返回值不改变。动态光照每次 50 ms 更新、实体移除或全量清空时递增纪元，旧缓存因此失效。线程本地实例避免渲染工作线程之间共享可变数组，也不需要锁或 GL 调用。

缓存没有单独的配置项：关闭既有 `dynamicLights` 开关即完全绕过该分支并保持原版路径；缓存只在现有 `DynamicLights.getCount() > 0` 条件成立时使用。`WorldMixin` 的世界直连路径本轮保持不变，避免同时改动两个公共 Mixin 入口。

## 风险边界

- **线程**：缓存是 `ThreadLocal`，纪元是 volatile；动态光照快照仍由现有不可变列表提供。查询在更新边界重新检查纪元，跨边界的缓存项会安全未命中并重算。
- **GL**：缓存不调用 OpenGL，只包裹 CPU 侧亮度计算；`ChunkCacheMixin` 仍在原调用返回后修改整数。
