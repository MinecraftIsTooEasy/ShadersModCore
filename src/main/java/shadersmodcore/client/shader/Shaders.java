...
public class Shaders {
    ...
    public static void loadShaderPack() {
        ...
        shaderActive = !packName.isEmpty() && !packName.equals(packNameNone);
    }
    ...
    static ArrayList listofShaders() {
        ...
        try {
            ...
            File[] var2 = listOfFiles;
            int var3 = listOfFiles.length;
    ...
    public static void init() throws NoSuchFieldException, IllegalAccessException {
        if (!isInitialized) {
            ...
            for(n = 0; n < 21; ++n) {
                if (programNames[n] == "") {
                    programsID[n] = programsRef[n] = 0;
                    programsDrawBufSettings[n] = null;
                    programsColorAtmSettings[n] = null;
                    programsCompositeMipmapSetting[n] = 0;
                } else {
                    newDrawBufSetting = null;
                    newColorAtmSetting = null;
                    newCompositeMipmapSetting = 0;
                    p = setupProgram(n, "/shaders/" + programNames[n] + ".vsh", "/shaders/" + programNames[n] + ".fsh");
                    programsID[n] = programsRef[n] = p;
                    programsDrawBufSettings[n] = p != 0 ? newDrawBufSetting : null;
                    programsColorAtmSettings[n] = p != 0 ? newColorAtmSetting : null;
                    programsCompositeMipmapSetting[n] = p != 0 ? newCompositeMipmapSetting : 0;
                }
            }
            ...
            resetDisplayList();
            if (notFirstInit) {
                mc.ingameGUI.getChatGUI().printChatMessage(I18n.getString("shaders.load.success") + currentshadername);
            }
    ...
    private static int getUniformLocation(int program, String name) {
        ...
        Integer loc = uniformLocationCache.get(key);
        if (loc == null) {
            loc = ARBShaderObjects.glGetUniformLocationARB(program, name);
            uniformLocationCache.put(key, loc);
        }
        return loc;
    }
    ...
    public static void beginRender(Minecraft minecraft, float f, long l) throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, NoSuchFieldException {
        if (!isShadowPass) {
            ...
            float temp1 = (float)Math.exp(Math.log(0.5D) * (double)fadeScalar / (double)(wetness < rainStrength ? drynessHalfLife : wetnessHalfLife));
            ...
            isEyeInWater = mc.gameSettings.thirdPersonView == 0 && !mc.renderViewEntity.inBed() && mc.thePlayer.isInsideOfMaterial(Material.water) ? 1 : 0;
    ...
    public static void setCamera(float f) {
        ...
        cameraPosition[1] = y;
        cameraPosition[2] = z;
        GL11.glGetFloat(2983, projection.position(0));
        invertMat4x(projection.position(0), projectionInverse.position(0));
        projection.position(0);
        projectionInverse.position(0);
        GL11.glGetFloat(2982, modelView.position(0));
        invertMat4x(modelView.position(0), modelViewInverse.position(0));
        modelView.position(0);
        ...
        if (isShadowPass) {
            ...
            GL11.glGetFloat(2983, shadowProjection.position(0));
            invertMat4x(shadowProjection.position(0), shadowProjectionInverse.position(0));
            shadowProjection.position(0);
            shadowProjectionInverse.position(0);
            GL11.glGetFloat(2982, shadowModelView.position(0));
            invertMat4x(shadowModelView.position(0), shadowModelViewInverse.position(0));
    ...
    public static void preSkyList() {
        ...
        tess.addVertex(-256.0D, -64.0D, -384.0D);
        tess.addVertex(-256.0D, 16.0D, -384.0D);
        tess.addVertex(-384.0D, 16.0D, -256.0D);
        tess.addVertex(-384.0D, -64.0D, -256.0D);
        tess.addVertex(-384.0D, -64.0D, -256.0D);
        tess.addVertex(-384.0D, 16.0D, -256.0D);
        tess.addVertex(-384.0D, 16.0D, 256.0D);
        tess.addVertex(-384.0D, -64.0D, 256.0D);
        tess.addVertex(-384.0D, -64.0D, 256.0D);
        tess.addVertex(-384.0D, 16.0D, 256.0D);
        tess.addVertex(-256.0D, 16.0D, 256.0D);
        tess.addVertex(-256.0D, -64.0D, 256.0D);
        tess.addVertex(-256.0D, -64.0D, 256.0D);
        tess.addVertex(-256.0D, 16.0D, 256.0D);
        tess.addVertex(256.0D, 16.0D, 384.0D);
        tess.addVertex(256.0D, -64.0D, 384.0D);
        ...
        tess.addVertex(-256.0D, 16.0D, -384.0D);
        tess.addVertex(-256.0D, -64.0D, -384.0D);
        tess.addVertex(-384.0D, -64.0D, -384.0D);
        tess.addVertex(-384.0D, -64.0D, 384.0D);
        tess.addVertex(384.0D, -64.0D, 384.0D);
        tess.addVertex(384.0D, -64.0D, -384.0D);
        tess.draw();
    ...
    public static boolean shouldRenderClouds(GameSettings gs) {
        ...
        return isShadowPass ? configCloudShadow : gs.shouldRenderClouds();
    }
    ...
    public static void endTerrain() {
        if (isRenderingWorld) {
            useProgram(lightmapEnabled ? 3 : 2);
            ShadersTex.bindNSTextures(((AbstractTextureAccessor) ReflectHelper.dyCast(defaultTexture)).getMultiTexID());
        }
